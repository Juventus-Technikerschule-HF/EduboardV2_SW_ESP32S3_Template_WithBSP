...
uint8_t sinedata[] = {0xFF,0xFF,0xFF,0xC6,0xD9,0xE9,0xF4,0xFC,0xFE,0xFC,0xF4,0xE9,0xD9,0xC6,0xB0,0x98,
                        0x7F,0x66,0x4E,0x38,0x25,0x15,0xA,0x2,0x0,0x2,0xA,0x15,0x25,0x38,0x4E,0x66,
                        0x7F,0x98,0xB0,0xC6,0xD9,0xE9,0xF4,0xFC,0xFE,0xFC,0xF4,0xE9,0xD9,0xC6,0xB0,0x98,
                        0x7F,0x66,0x4E,0x38,0x25,0x15,0xA,0x2,0x0,0x2,0xA,0x15,0x25,0x38,0x4E,0x66,
                        0x7F,0x98,0xB0,0xC6,0xD9,0xE9,0xF4,0xFC,0xFE,0xFC,0xF4,0xE9,0xD9,0xC6,0xB0,0x98,
                        ...
                        0x7F,0x66,0x4E,0x38,0x25,0x15,0xA,0x2,0x0,0x2,0xA,0x15,0x25,0x38,0x4E,0x66,
                        0x7F,0x98,0xB0,0xC6,0xD9,0xE9,0xF4,0xFC,0xFE,0xFC,0xF4,0xE9,0xD9,0xC6,0xB0,0x98,
                        0x7F,0x66,0x4E,0x38,0x25,0x15,0xA,0x2,0x0,0x2,0xA,0x15,0x25,0xFF,0xFF,0xFF,};

#define UPDATETIME_MS 100
uint16_t databuffer[32];
uint8_t outputbuffer[32];
void dacCallbackFunction() {
    // dac_load_stream_data(sinedata, outputbuffer);
    dac_load_stream_data(outputbuffer, sinedata);
    ESP_LOGI(TAG, "DAC_UPDATE");
}

void adcCallbackFunction() {
    adc_get_buffer(AN2, &databuffer[0]);
    for(int i = 0; i < 32; i++) {
        outputbuffer[i] = ((databuffer[i]) >> 4)*1.6;
    }
    ESP_LOGI(TAG, "ADC_UPDATE");
}
...
void templateTask(void* param) {
    ...
    for(;;) {
        uint32_t adcValue = adc_get_raw(AN1);
        uint32_t adcVoltage_mv = adc_get_voltage_mv(AN1);
